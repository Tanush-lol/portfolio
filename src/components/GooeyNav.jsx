...
const GooeyNav = ({
  ...
  // ⬇️ Navigation now WAITs for animation to finish before router.push()
  const handleNav = (href, index) => {

    // Skip if clicking current page -> no animation needed
    if (href === pathname) return;

    setActiveIndex(index);

    updateEffectPosition(navRef.current.querySelectorAll("li")[index]);
    ...
    // ⬇️ 🔥 NEW: Delay navigation by 1 second (1000ms)
    setTimeout(() => {
      router.push(href); // manually navigate after animation
    }, 2500);
  };
  return (
    <>
